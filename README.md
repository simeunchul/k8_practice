...
>
> **📍 현재 상태(정직 표기)**: **Phase 0(Kafka producer/consumer group·오프셋 커밋) + Phase 2 경량 경로(kind 로컬 K8s에 Kafka+producer+consumer를 Deployment/Service로 배포·운영, 스케일·롤아웃 포함) 실행·검증 완료** ([k8s/local/](k8s/local/) 실행 증거 포함). Phase 1(Spark)·Phase 3(Airflow)·Phase 4(EKS)와 Phase 2 전체 경로(Strimzi+Spark Operator)는 코드 작성 단계로 **학습·검증 진행 중**(아래 로드맵 상태 참고). AI로 구조를 먼저 잡고, 각 단계를 직접 돌려보며 동작·장애·튜닝을 이해하는 것이 학습 목표.
...
| **Phase 2** | kind 위로 — (경량) Kafka+producer+consumer를 Deployment/Service로 배포·운영 / (전체) Strimzi+Spark Operator | [k8s/local/](k8s/local/), [k8s/](k8s/) | **K8s** | ✅ 경량 경로 실행·검증 완료 / 🚧 전체 경로 코드 초안 |
